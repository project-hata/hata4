...
instance noMaxOrder [PartialOrder α] [NoMaxOrder α] {a : α} : NoMaxOrder { x : α // a ≤ x } :=
  {
    exists_gt := λ x =>
      have ⟨ a' , a'p ⟩ := x
      have ⟨ b' , b'p ⟩ := exists_gt a'
      ⟨ ⟨ b' , le_of_lt (trans a'p b'p) ⟩ , by simp [b'p] ⟩
  }
...
instance denselyOrdered [Preorder α] [DenselyOrdered α] {a : α} : DenselyOrdered { x : α // a ≤ x } where
  dense (a₁ a₂ : { x // a ≤ x }) (p1 : a₁ < a₂) : ∃ a_2, a₁ < a_2 ∧ a_2 < a₂
   := have ⟨ a_2' , p2 ⟩ := DenselyOrdered.dense a₁.val a₂.val (by assumption)
      have P2 : a ≤ a_2' := Trans.trans a₁.property (le_of_lt p2.1)
      ⟨ ⟨ a_2' , P2 ⟩ , ⟨ p2.1 , p2.2 ⟩ ⟩
...
instance hasNsmul [AddMonoid α] [Preorder α] [CovariantClass α α (· + ·) (· ≤ ·)] :
    SMul ℕ { x : α // 0 ≤ x } :=
  ⟨fun n x => ⟨n • x.val, nsmul_nonneg x.property n⟩⟩
...
def coeAddMonoidHom [OrderedAddCommMonoid α] : { x : α // 0 ≤ x } →+ α where
  toFun := Subtype.val
  map_zero' := Nonneg.coe_zero
  map_add' := Nonneg.coe_add
  -- ⟨Subtype.val, Nonneg.coe_zero, Nonneg.coe_add⟩
...
instance addMonoidWithOne [P: OrderedSemiring α] : AddMonoidWithOne { x : α // 0 ≤ x } :=
  { Nonneg.hasOne,
    Nonneg.orderedAddCommMonoid 
      with
    natCast := fun n => ⟨n, Nat.cast_nonneg n⟩
    natCast_zero := by simp [P.natCast_zero]
    -- sorry -- by simp [Nat.cast, natCast_zero]
    -- natCast_zero := by simp [Nat.cast]
    natCast_succ := fun _ => by simp [P.natCast_succ] ; rfl
  }
...
instance hasPow [OrderedSemiring α] : Pow { x : α // 0 ≤ x } ℕ
    where pow x n := ⟨x.val ^ n, pow_nonneg x.2 n⟩
...
def coeRingHom [OrderedSemiring α] : { x : α // 0 ≤ x } →+* α where
  toFun := Subtype.val
