...
structure MeasurableSpace (α : Type _) where
  ...
  measurable_set_empty : MeasurableSet' ∅
  measurable_set_compl : ∀ s, MeasurableSet' s → MeasurableSet' (sᶜ)
  measurable_set_Union : ∀ f : ℕ → Set α, (∀ i, MeasurableSet' (f i)) → MeasurableSet' (⋃ i, f i)
...
theorem measurableSet_supₛ {ms : Set (MeasurableSpace α)} {s : Set α} :
  ...
  by
  change @MeasurableSet' _ (generate_from <| ⋃₀ _) _ ↔ _
