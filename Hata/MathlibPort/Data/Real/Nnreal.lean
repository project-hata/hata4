...
import Hata.MathlibPort.Algebra.Algebra.Basic
import Mathlib.Data.Real.Basic
import Hata.MathlibPort.Algebra.Order.Nonneg.Field
import Hata.MathlibPort.Algebra.Order.Nonneg.Ring
...
open Classical Real Nonneg -- BigOperators

-- instance : DenselyOrdered ℝ := inferInstance
-- instance : DenselyOrdered { r : ℝ // 0 ≤ r } := inferInstance
-- example : LinearOrderedRing ℝ := inferInstance

-- to ensure these instances are computable
/-- Nonnegative real numbers. -/
def Nnreal :=
  { r : ℝ // 0 ≤ r }deriving StrictOrderedSemiring, CommMonoidWithZero, -- FloorSemiring,
   CommSemiring,
  Semiring, SemilatticeInf, SemilatticeSup, DistribLattice --, DenselyOrdered,
  --  OrderBot,
...
-- porting note (MxmUrw): somehow the deriving statement doesn't work, so we use
-- simple instance statements
instance : DenselyOrdered Nnreal := denselyOrdered
noncomputable instance : FloorSemiring Nnreal := floorSemiring
instance : OrderBot Nnreal := orderBot
noncomputable instance : CanonicallyLinearOrderedSemifield Nnreal := canonicallyLinearOrderedSemifield
noncomputable instance : LinearOrderedCommGroupWithZero Nnreal := linearOrderedCommGroupWithZero
instance : Archimedean Nnreal := archimedean
noncomputable instance : LinearOrderedSemiring Nnreal := linearOrderedSemiring
instance : OrderedCommSemiring Nnreal := orderedCommSemiring
instance : CanonicallyOrderedCommSemiring Nnreal := canonicallyOrderedCommSemiring
noncomputable instance : Sub Nnreal := Nonneg.hasSub
noncomputable instance : OrderedSub Nnreal := Nonneg.hasOrderedSub
noncomputable instance : Div Nnreal := Nonneg.hasDiv
instance : Inhabited Nnreal := Nonneg.inhabited
...
noncomputable example : Zero ℝ≥0 := by infer_instance
...
protected theorem coe_injective : Function.Injective (Coe.coe : ℝ≥0 → ℝ) :=
...
@[simp, norm_cast]
protected theorem coe_bit0 (r : ℝ≥0) : ((bit0 r : ℝ≥0) : ℝ) = bit0 r.val :=
  rfl
#align nnreal.coe_bit0 Nnreal.coe_bit0

@[simp, norm_cast]
protected theorem coe_bit1 (r : ℝ≥0) : ((bit1 r : ℝ≥0) : ℝ) = bit1 r.val :=
  rfl
...
protected theorem coe_sub {r₁ r₂ : ℝ≥0} (h : r₂ ≤ r₁) : ((r₁ - r₂ : ℝ≥0) : ℝ) = r₁ - r₂ :=
  max_eq_left <| le_sub_comm.2 <| sorry -- by simp [show (r₂ : ℝ) ≤ r₁ from h]
...
theorem coe_ne_zero {r : ℝ≥0} : (r : ℝ) ≠ 0 ↔ r ≠ 0 := sorry -- by norm_cast
...
def toRealHom : ℝ≥0 →+* ℝ :=
  ⟨⟨⟨Coe.coe, Nnreal.coe_one⟩, Nnreal.coe_mul⟩, Nnreal.coe_zero, Nnreal.coe_add⟩
